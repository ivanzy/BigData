...
import Data.List
--new data types
data Color = Yellow | Red | Blue | Ivory | Green deriving (Show,  Eq, Enum, Ord, Bounded)
data Drink = Tea | Milk | Coffee | Water | Orange deriving (Show,  Eq, Enum, Ord, Bounded)
data Pet = Zebra | Dog | Snails | Fox | Horse deriving (Show,  Eq, Enum, Ord, Bounded)
data Nationality = Norwegian | Japanese | Ukrainian | Spaniard | Englishman deriving (Show,  Eq, Enum, Ord, Bounded)
data Smoke = LuckyStrike | OldGold | Parliament | Kools | Chesterfield deriving (Show,  Eq, Enum, Ord, Bounded)
data House = House Color Nationality Pet Drink Smoke deriving (Show,  Eq, Ord, Bounded)


cartProd cs ns ps ds ss = [ [House c n p d s,House c n p d s,House c n p d s,House c n p d s,House c n p d s] | c <- cs, n <- ns, p <- ps, d <- ds, s <-ss]
